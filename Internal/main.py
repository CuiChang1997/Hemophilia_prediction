...
data = np.genfromtxt("data_internal.csv", delimiter=',')
...
for i, (train_index, test_index) in enumerate(kf.split(data)):
    ...
    model.fit(x_train, y_train, 32, 100, [tt.callbacks.EarlyStopping()], False, val_data=val)
    ...
    test = x_test, y_test
    net = tt.practical.MLPVanilla(x_train.shape[1], [128, 128], 1, True, 0.1, output_bias=False)
    model = CoxPH(net, tt.optim.Adam)
    model.optimizer.set_lr(0.001)
    model.fit(x_train, y_train, 128, 512, [tt.callbacks.EarlyStopping()], False, val_data=test, val_batch_size=128)
    model.compute_baseline_hazards()
    surv = model.predict_surv_df(x_test)
    ev = EvalSurv(surv, t_test, e_test, censor_surv='km')
    c_index = ev.concordance_td()
    ...
    if i == 4:
        ...
        rsf = RandomSurvivalForest(n_estimators=400,
                                   max_features="sqrt",
                                   n_jobs=-1,
                                   random_state=1)
